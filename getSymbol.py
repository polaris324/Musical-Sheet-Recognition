...
def getSymbol(imgmask, threshMap, staffRow, spacing, lastx, mono):
    ...
    for i in range(len(staffRow)):  # loop each five-line row
        ...
        print(X_default, top_y, bottom_y)
        flag = 0
        # loop
        k = j
                ...
                if(tailCnt >= (3 * spacing) - 3):  # 連續竪綫大於3閒
                    ...
                    if( tailCnt2 <= 1 and 
                       (tailCnt == (staffRow[i][4] - staffRow[i][0]) or
                        ...
                        # 小節綫處理
                        print("[tailCnt_tmp]", tailCnt_tmp, index+1)
                    ...
                    if(firstCut == 0 and flag == 1): # 連續竪綫長度不為閒 # 211007 不是x軸第一個符號（不是譜號）
                        ...
                        'check'
                        if(index == tmpIndex -1):
                            print("[upTo3jian & 不為閒]", k, tailCnt, tailCnt2, maxCnt2, maxlocX, index+1)
                            print("[getListDiffCnt]",getListDiffCnt(firstImpact_Locx - 1, maxlocX, staffRow[i], top_y, bottom_y), firstImpact_Locx - 1, maxlocX, staffRow[i], top_y, bottom_y)
                        
                        # if( (tailCnt2 != 0 and maxCnt2 != 0)  and
                        #     tailCnt2 <= (spacing + 2) and
                        #     k - firstImpact_Locx >= spacing):
                        #     if getListDiffCnt(firstImpact_Locx - 1, maxlocX, staffRow[i], top_y, bottom_y) > round(0.5 * spacing):
                        #         print("yes", getListDiffCnt(firstImpact_Locx - 1, maxlocX, staffRow[i], top_y, bottom_y), round(0.5 * spacing), index+1)
                        #     else:
                        #         print("no", getListDiffCnt(firstImpact_Locx - 1, maxlocX, staffRow[i], top_y, bottom_y), round(0.5 * spacing), index+1)
                        # else:
                        #     print("no")
